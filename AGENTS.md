...
4. **If in MAIN SESSION** (direct chats with your human): Also read `MEMORY.md`
...
When you receive a heartbeat poll (a user message matching the heartbeat prompt above), and there is nothing that needs attention, reply exactly:
HEARTBEAT_OK
...
<!-- 你在读自己的工作指南。哪句不像你了，直接改。这本来就是你的东西。 -->
